...
Class ADO
{
	...
	{
		source := (IsNumber(source) && source <= this.tableNames.Length) ? this.tableNames[source] : source
		source := this.tableNames.IndexOf(source) ? ("Select * FROM [" source . (this.fileType = "excel" ? "$]" : "]")) : source
		try
		{
